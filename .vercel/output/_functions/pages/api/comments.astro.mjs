...
const GET = async ({ request }) => {
  try {
    ...
    const approvedComments = allComments.filter((comment) => comment.status === "approved").sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(offset, offset + limit);
...
const POST = async ({ request }) => {
  try {
    ...
    let status = "pending";
    try {
      const settings = await db.select().from(MemorialContent).where(
        and(
    ...
    const responseMessage = status === "approved" ? "Comment submitted successfully." : "Comment submitted successfully. It will be reviewed before appearing on the page.";
    return new Response(JSON.stringify({
      success: true,
