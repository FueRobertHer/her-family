...
export function optimizeCloudinaryPdf(url: string): string {
  ...
  // and then ignores it. Verified against res.cloudinary.com - the transformed
  // and untransformed raw URLs return byte-identical responses. So skip it and
  // keep the URL short; only a PDF stored as an image asset can be optimized.
